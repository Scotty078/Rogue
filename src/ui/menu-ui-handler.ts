...
export default class MenuUiHandler extends MessageUiHandler {
  ...
  render() {
    ...
    this.walletStatusText = addTextObject(
      globalScene.game.canvas.width / 6 - 10,
      10,
      this.wallet.isConnected() ? "Wallet Connected" : "Connect Wallet",
      TextStyle.WINDOW,
      { fontSize: "32px" }
    );
    this.walletStatusText.setOrigin(1, 0);
    this.menuContainer.add(this.walletStatusText);
  }
    ...
    if (button === Button.ACTION) {
      ...
      switch (adjustedCursor) {
        ...
        case MenuOptions.LOG_OUT:
          ...
          break;
        case MenuOptions.WALLET:
          success = true;
          if (!this.wallet.isConnected()) {
            this.wallet.connect().then(connected => {
              if (connected) {
                this.walletStatusText.setText("Wallet Connected");
                ui.showText("Wallet connected successfully!", null, () => {
                  ui.revertMode();
                });
              } else {
                ui.showText("Failed to connect wallet. Please install Phantom wallet.", null, () => {
                  ui.revertMode();
                });
              }
            }).catch(error => {
              ui.showText("Failed to connect wallet: " + error.message, null, () => {
                ui.revertMode();
              });
            });
          } else {
            this.wallet.disconnect().then(() => {
              this.walletStatusText.setText("Connect Wallet");
              ui.showText("Wallet disconnected", null, () => {
                ui.revertMode();
              });
            }).catch(error => {
              ui.showText("Failed to disconnect wallet: " + error.message, null, () => {
                ui.revertMode();
              });
            });
          }

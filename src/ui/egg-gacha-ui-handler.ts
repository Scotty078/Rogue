    ...
    const pullOptions = [
      {
        ...
        description: `1 ${i18next.t("egg:pull")} (${this.solPayments.getVoucherPrice(VoucherType.REGULAR)} SOL)`,
        icon: getVoucherTypeIcon(VoucherType.REGULAR),
      },
      {
        multiplier: multiplierTen,
        description: `10 ${i18next.t("egg:pulls")} (${this.solPayments.getVoucherPrice(VoucherType.REGULAR) * 10} SOL)`,
        icon: getVoucherTypeIcon(VoucherType.REGULAR),
      },
      {
        multiplier: multiplierOne,
        description: `5 ${i18next.t("egg:pulls")} (${this.solPayments.getVoucherPrice(VoucherType.PLUS)} SOL)`,
        icon: getVoucherTypeIcon(VoucherType.PLUS),
      },
      {
        multiplier: multiplierOne,
        description: `10 ${i18next.t("egg:pulls")} (${this.solPayments.getVoucherPrice(VoucherType.PREMIUM)} SOL)`,
        icon: getVoucherTypeIcon(VoucherType.PREMIUM),
      },
      {
        multiplier: multiplierOne,
        description: `25 ${i18next.t("egg:pulls")} (${this.solPayments.getVoucherPrice(VoucherType.GOLDEN)} SOL)`,
            ...
            switch (this.cursor) {
              case 0:
                if (globalScene.gameData.eggs.length < 99 || Overrides.UNLIMITED_EGG_COUNT_OVERRIDE) {
                  this.solPayments.purchaseVoucher(VoucherType.REGULAR)
                    .then(success => {
                      if (success) {
                        this.pull();
                      } else {
                        this.showError(i18next.t("egg:paymentFailed"));
                      }
                    })
                    .catch(error => {
                      console.error('Voucher purchase error:', error);
                      this.showError(error.message || i18next.t("egg:paymentFailed"));
                      // Reset UI state
                      this.eggGachaOptionsContainer.setVisible(true);
                    });
                  success = true;
                ...
                break;
              case 2:
                if (globalScene.gameData.eggs.length < 95 || Overrides.UNLIMITED_EGG_COUNT_OVERRIDE) {
                  this.solPayments.purchaseVoucher(VoucherType.PLUS)
                    .then(success => {
                      if (success) {
                        this.pull(5);
                      } else {
              ...
              case 1:
              case 3:
                if (globalScene.gameData.eggs.length < 90 || Overrides.UNLIMITED_EGG_COUNT_OVERRIDE) {
                  const voucherType = this.cursor === 3 ? VoucherType.PREMIUM : VoucherType.REGULAR;
                  const count = this.cursor === 3 ? 1 : 10;
                  this.solPayments.purchaseVoucher(voucherType)
                    .then(success => {
                      if (success) {
                        this.pull(10);
                      } else {
                ...
                break;
              case 4:
                if (globalScene.gameData.eggs.length < 75 || Overrides.UNLIMITED_EGG_COUNT_OVERRIDE) {
                  this.solPayments.purchaseVoucher(VoucherType.GOLDEN)
                    .then(success => {
                      if (success) {
                        this.pull(25);
                      } else {
